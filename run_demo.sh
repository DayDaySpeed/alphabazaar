#!/usr/bin/env bash
# AlphaBazaar one-command demo: start the seller agents, run the analyst, stop.
set -euo pipefail
cd "$(dirname "$0")"
...
MOMENTUM_PORT="${MOMENTUM_PORT:-8803}"
...
RISK_PID=$!
$PY -m uvicorn sellers.momentum_scanner:app --host 127.0.0.1 --port "$MOMENTUM_PORT" --log-level warning &
MOMENTUM_PID=$!

cleanup() {
  echo
  echo "▶ stopping seller agents…"
  kill "$FUNDING_PID" "$RISK_PID" "$MOMENTUM_PID" 2>/dev/null || true
  wait "$FUNDING_PID" "$RISK_PID" "$MOMENTUM_PID" 2>/dev/null || true
}
trap cleanup EXIT

for port in "$FUNDING_PORT" "$RISK_PORT" "$MOMENTUM_PORT"; do
...
echo "▶ sellers up on :$FUNDING_PORT :$RISK_PORT :$MOMENTUM_PORT"
echo

SELLER_FUNDING_URL="http://127.0.0.1:${FUNDING_PORT}" \
SELLER_RISK_URL="http://127.0.0.1:${RISK_PORT}" \
SELLER_MOMENTUM_URL="http://127.0.0.1:${MOMENTUM_PORT}" \
